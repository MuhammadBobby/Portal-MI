                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr class="leading-relaxed text-center border-b">
                                ...
                                <td class="px-4 py-3 {{ $item->role == 'admin' ? 'text-primary' : 'text-yellow-500' }}">
                                    {{ $item->role }}
                                </td>
                                <td class="flex items-center justify-end px-4 py-3">
                                    ...
                                    <div id="{{ $item->email }}"
                                        ...
                                        <ul class="py-1 text-sm text-gray-700"
                                            ...
                                            <li>
                                                <a href="/admin/users/{{ $item->email }}?callbackUrl={{ urlencode(url()->full()) }}"
                                                    class="block px-4 py-2 hover:bg-gray-100">Show</a>
                                            </li>
                                            <li>
                                                <a href="/admin/users/{{ $item->email }}/edit"
                                                    class="block px-4 py-2 hover:bg-gray-100">Edit</a>
                                            </li>
                                        </ul>
                                        <div class="py-1">
                                            <form id="deleteForm" action="/admin/users/{{ $item->email }}"
                                                method="POST"
